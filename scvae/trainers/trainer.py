...
class TrainerTemplate():
    ...
    def __init__(self,
                 ...
                 model_aux=None,
                 *,
                 disc_state_dict=None,  # only used in VQGAN trainers
                 ):
        super().__init__()
    ...
    def run_epoch(self, optimizer=None, scheduler=None, epoch_st=0):
        ...
        for i in range(epoch_st, self.config.experiment.epochs):
            ...
            self.train(optimizer, scheduler, scaler, epoch=i)
            if i == 0 or (i+1) % self.config.experiment.test_freq == 0:
                torch.cuda.empty_cache()
                Dictionary = self.eval(epoch=i)
                self.writer.add_image('Dictionary', Dictionary, i + 1, dataformats='HW')
