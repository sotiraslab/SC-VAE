...
def initialize(args, logger=None):
    ...
    print('rank:' + str(args.rank)); print('world size:' + str(args.world_size)); print('local rank:' + str(args.local_rank))

    if args.world_size > 1:   #then distibuted
...
def dataparallel_and_sync(distenv, model, find_unused_parameters=False):
    ...
    if dist.is_initialized():
        ...
        model = DistributedDataParallel(
            model, device_ids=[distenv.local_rank],
